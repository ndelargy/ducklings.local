(function (jQuery) {
  jQuery.fn.extend({h5w:function (options, val) {
    if (options == "destroy") {
    ...
    ;
    var options = jQuery.extend({onStart:function (teadasd) {
      return true;
    }, onChange:function () {
      return false;
    ...
    }, content:"", functions:{}, contextMenu:{table:function () {
    }}}, options);
    var DefineddContextMenu = {tableElement:function (ox, oy) {
      var hOContextMenu = $(".h5w-context-menu .h5w-c-options-element").html("");
      hOContextMenu.append($("<li>").html($("<a>").attr({"href":"#", "class":"h5w-c-deleterow"}).text("Delete row").click(function () {
        if ($(ox.target).is("tr")) {
          $(ox.target).remove();
        } else {
      ...
      hOContextMenu.append($("<li>").html($("<a>").attr({"href":"#", "class":"h5w-c-deletecell"}).text("Delete cell").click(function () {
        if ($(ox.target).is("td")) {
          $(ox.target).remove();
        }
      })));
    }};
    var contextMenu = {tr:DefineddContextMenu.tableElement, td:DefineddContextMenu.tableElement, };
    var functions = jQuery.extend({italic:function (main, icon) {
      ...
      jQuery(main).find(".h5w-content a[href='changeit']").attr({href:prompt("Please URL", "http://"), target:"_blank"});
      return true;
      ...
      jQuery(main).find(".h5w-content img[src='changeit']").each(function () {
        ...
        jQuery(this).attr({src:src, alt:alt});
    ...
    var tablepicker = {generate:function (width, height, anchor) {
      ...
      jQuery(MainHandle).find(".h5w-tablepicker").click(function () {
        ...
        jQuery(MainHandle).find(".h5w-tablepicker-area").css({left:position.left + positionEditor.left + jQuery(this).width() + 5, top:position.top + positionEditor.top + jQuery(this).height() + 5}).html(tablepicker.generate(16, 16, true)).show("slow").find("table").delegate('td', 'mouseover mouseleave',function (e) {
          if (e.type == 'mouseover') {
      ...
      ;
    }}, picker = {changebg:".something", color:'', onChange:function (color) {
      return false;
      ...
      jQuery(MainHandle).find(".h5w-picker-red, .h5w-picker-green, .h5w-picker-blue, .h5w-picker-opacity").slider({orientation:"horizontal", range:"min", max:255, value:127, slide:picker.refreshSwatch, change:picker.refreshSwatch});
      jQuery(MainHandle).find(".h5w-picker-red").slider("value", 255);
      jQuery(MainHandle).find(".h5w-picker-green").slider("value", 140);
      jQuery(MainHandle).find(".h5w-picker-green").slider("value", 140);
      jQuery(MainHandle).find(".h5w-picker-opacity").slider({max:100, value:100, });
      jQuery(MainHandle).find(".h5w-picker").click(function () {
        ...
        jQuery(MainHandle).find(".h5w-picker-area").css({left:position.left + positionEditor.left + jQuery(this).width() + 5, top:position.top + positionEditor.top + jQuery(this).height() + 5}).show("slow");
    ...
    sizepicker = {changesize:".something", fontSize:12, onChange:function (fontSize) {
      return false;
      ...
      jQuery(MainHandle).find(".h5w-fontsize-picker").click(function () {
        ...
        jQuery(MainHandle).find(".h5w-fontsize-picker-size").slider("destroy").slider({max:73, min:8, value:jQuery(sizepicker.changesize).text(), slide:sizepicker.refreshSize, change:sizepicker.refreshSize});
        var position = jQuery(this).position();
        var positionEditor = jQuery(MainHandle).position();
        sizepicker.changesize = jQuery(this).data("h5w-destination");
        sizepicker.onChange = eval(jQuery(this).data("h5w-onchange-function"));
        jQuery(MainHandle).find(".h5w-fontsize-picker-area").css({left:position.left + positionEditor.left + jQuery(this).width() + 5, top:position.top + positionEditor.top + jQuery(this).height() + 5}).show("slow");
    ...
    typepicker = {changeType:".something", fontType:"Arial", onChange:function (fontType) {
      return false;
    }, prepare:function (MainHandle) {
      jQuery(MainHandle).find(".h5w-fonttype-picker").click(function () {
        ...
        jQuery(MainHandle).find(".h5w-font-picker-area").css({left:position.left + positionEditor.left + jQuery(this).width() - 100, top:position.top + positionEditor.top + jQuery(this).height() + 10}).show("slow");
    ...
    function selectElement(event) {
      ...
      if (jTarget.parents('.h5w-content').length || jTarget.is(".h5w-content")) {
        ...
        jQuery(".h5w-fonttype span").css("font-family", FontFamily).text(FontShortName[0]);
        jQuery(".h5w-fontsize").text(jTarget.css("font-size").replace("px", ""));
    ...
    return this.each(function () {
      ...
      jQuery(MainHandle).find('.h5w-tabs').tabs({}).find(".ui-tabs-nav").sortable({axis:"x"});
      ...
      jQuery(MainHandle).find(".h5w-content").html(options.content).bind("keyup change", options.onChange).click(selectElement).bind('contextmenu', function (e) {
        ...
        ;
        $(MainHandle).find(".h5w-context-menu").css({left:e.pageX, top:e.pageY, zIndex:'101'}).show().click("click", function () {
          $(this).hide("slow");
        });
        ...
        if (!HContextAttr.is(".h5w-content")) {
          ...
          hContextMenu.append($("<li>").html($("<a>").attr({"href":"#", "class":"h5w-c-add"}).text("Add").click(function () {
          ...
          for (var i = 0, attrs = e.target.attributes, l = attrs.length; i < l; i++) {
            hContextMenu.append($("<li>").html($("<a>").attr({"href":"#", "class":"h5w-c-attrx"}).text(attrs.item(i).nodeName).click(function () {
